...
Annotations helps to document possible values that can be passed as a parameter's value.
...
We plan to support various kinds of annotations in the future; for now, only `[number, number]` tuples are supported, which are interpreted as numeric ranges.
