...
Creates a [normal distribution](https://en.wikipedia.org/wiki/Normal_distribution) with the given mean and standard deviation. This can be dangerous if the shape is not normalized; if this is the case, you should normalize it (``normalize(dist)``) before using.
...
<SquiggleEditor defaultCode="Sym.exponential(4)" />
...
<Callout type="warning">
  **Caution!**  
  Distributions made with ``makeContinuous`` are not normalized. We suggest normalizing them manually using the ``normalize`` function.
</Callout>
