...
<SquiggleEditor
  defaultCode={`normal(5,2) -> truncateLeft(3) -> SampleSet.fromDist -> SampleSet.map({|bar| bar + 10})`}
/>

// `bar` is the temporary binding variable used in the SampleSet.map function. It
// represents each element drawn from a SampleSet created by `SampleSet.fromDist` after
// it has been truncated by `truncateLeft`. The `bar + 10` operation within the map
// demonstrates how to perform a transformation on each element, in this case, adding 10.
// Type: Element of the SampleSet after truncation
// Usage: Within `SampleSet.map`, define a block with `bar` as the parameter to perform operations on each element.
