...
<SquiggleEditor
  defaultCode={`multiplyTwoNumbers(t) = normal(t^2, t^1.2+.01)
multiplyTwoNumbers`}
/>
...
<SquiggleEditor
  defaultCode={`multiplyInputBySix(x) = {
  doubleX = x * 2
  doubleX * 3
  }`}
/>
...
In Squiggle, you can define anonymous functions using the `{|...| ...}` syntax. For example, `multiplyTwoNumbers(x, y) = x * y` and `multiplyTwoNumbers = {|x, y| x * y}` are equivalent.
...
<SquiggleEditor
  defaultCode={`multiplyTwoNumbers(x: [1, 20]) = x * x
...
<SquiggleEditor
  defaultCode={`multiplyTwoNumbers(x: [1, 20]) = x * x
g(x: multiplyTwoNumbers.parameters[0].domain) = x * x * x
`}
/>