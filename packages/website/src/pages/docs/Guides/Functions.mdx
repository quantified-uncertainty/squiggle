...
<SquiggleEditor
  defaultCode={`multiplyByThreeTwice(x) = {
  timesTwo = x * 2
  timesTwo * 3
  }`}
/>
...
In Squiggle, you can define anonymous functions using the `{|...| ...}` syntax. For example, `product(x, y) = x * y` and `product = {|x, y| x * y}` are equivalent.
...
<SquiggleEditor defaultCode={`{|time| normal(time^2, time^1.2+.01)}`} />
...
<SquiggleEditor
  defaultCode={`product(x: [1, 20]) = x * y
...
<SquiggleEditor
  defaultCode={`product(x: [1, 20]) = x * y
g(x: product.parameters[0].domain) = x * x * x
`}
/>