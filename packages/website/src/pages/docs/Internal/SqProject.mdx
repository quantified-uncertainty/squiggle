...
It allows you to customize the way Squiggle modules are resolved and loaded.
...
Imports look like this: `import "import-name" as importVar`.

In this example, `"import-name"` is a string that will first be resolved by the `linker.resolve` method to the real module name.

This is useful because in the command-line environment you might prefer to do relative imports: `import "./foo" as foo`, and we resolve that to the absolute path. Otherwise you'd get duplicate instances of the same module.

After that, the module is loaded with `linker.loadModule` method. This module takes two parameters: the module name and its optional hash that comes from pins stored on the parent module object.

If the hash is provided, the linker **must** return the module with that hash.

If **all** modules have pins, all the way down to the leaf modules, the project will be completely deterministic and reproducible.
