...
function setHashData(str: string) {
  const text = JSON.stringify({ initialSquiggleString: str });
    ...
    return (
      <div>
        <div className="mt-2 mb-6 flex overflow-x-auto items-center p-1">
          ...
          <span className="text-sm bg-slate-200 bg-opacity-80 rounded-sm text-slate-900 px-1 text-center whitespace-pre-wrap mr-2 ml-2">
            {numeratorItem.name}
          </span>
        ...
        <a href={setHashData(squggleCode)} className="text-slate-400">
          Open in Playground
        </a>
      </div>
    );
