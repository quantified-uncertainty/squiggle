...
comment_weight = karma_weight * (3 to 20)

score(importance, karma, comments) = (importance^(1.5 to 3.5)  * importance_weight) + (karma) + (comments * comment_weight)

scoreById(id) = {
    item = data[id]
    score(item.importance, item.karma, item.commentCount) * (1 to 100)
}
...
export function getQuriPosts(): InterfaceWithModels {
  return {
    catalog: getCatalog(),
    models: Map([["QURI Papers Estimate, Quick, by Ozzie", getTextModel()]]),
  };
}
