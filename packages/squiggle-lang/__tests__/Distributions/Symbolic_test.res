...
describe("(Symbolic) mean", () => {
  testAll("of normal distributions", list{-1e8, -16.0, -1e-2, 0.0, 1e-4, 32.0, 1e16}, mean => {
    run(#ToFloat(#Mean), mkNormal(mean, 4.0))->unpackFloat->expect->toBeCloseTo(mean)
  })

  Skip.test("of normal(0, -1) (it NaNs out)", () => {
    run(#ToFloat(#Mean), mkNormal(1e1, -1e0))->unpackFloat->expect->ExpectJs.toBeFalsy
  })

  test("of normal(0, 1e-8) (it doesn't freak out at tiny stdev)", () => {
    run(#ToFloat(#Mean), mkNormal(0.0, 1e-8))->unpackFloat->expect->toBeCloseTo(0.0)
  })

  testAll("of exponential distributions", list{1e-7, 2.0, 10.0, 100.0}, rate => {
    let meanValue = run(#ToFloat(#Mean), DistributionTypes.Symbolic(#Exponential({rate: rate})))
  ...
  test("of a cauchy distribution", () => {
    let meanValue = run(
      #ToFloat(#Mean),
      DistributionTypes.Symbolic(#Cauchy({local: 1.0, scale: 1.0})),
    )
  ...
  testAll(
    ...
    tup => {
      ...
      let meanValue = run(
        #ToFloat(#Mean),
        DistributionTypes.Symbolic(#Triangular({low, medium, high})),
      )
  ...
  testAll(
    ...
    tup => {
      let (alpha, beta) = tup
      let meanValue = run(#ToFloat(#Mean), DistributionTypes.Symbolic(#Beta({alpha, beta})))
  ...
  test("of beta(0, 0)", () => {
    let meanValue = run(#ToFloat(#Mean), DistributionTypes.Symbolic(#Beta({alpha: 0.0, beta: 0.0})))
  ...
  testAll(
    ...
    tup => {
      ...
      let meanValue =
        betaDistribution->E.R.fmap(d => run(#ToFloat(#Mean), d->DistributionTypes.Symbolic))
      switch meanValue {
  ...
  testAll(
    ...
    tup => {
      let (mu, sigma) = tup
      let meanValue = run(#ToFloat(#Mean), DistributionTypes.Symbolic(#Lognormal({mu, sigma})))
  ...
  testAll(
    ...
    tup => {
      let (low, high) = tup
      let meanValue = run(#ToFloat(#Mean), DistributionTypes.Symbolic(#Uniform({low, high})))
  ...
  test("of a float", () => {
    let meanValue = run(#ToFloat(#Mean), DistributionTypes.Symbolic(#Float(7.7)))
