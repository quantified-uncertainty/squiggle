  ...
  console.log(
    prefix +
      ...
      varMatrix
        .map(
          (row, i) =>
            row.map((x) => String(x).padStart(2, " ")).join(" ") +
            " + " +
            unitMatrix[i].map((x) => String(x).padStart(2, " ")).join(" ")
        )
        .join("\n\t")
  );
}
...
/*
 * (deprecated) Solve type inference by running Gaussian elimination on the
 * matrix of variables.
 *
...
function gaussianElim(
    ...
    for (let j = 0; j < varMatrix[i].length; j++) {
      if (varMatrix[i][j] !== 0) {
        for (let k = i - 1; k >= 0; k--) {
          ...
          unitMatrix[k] = unitMatrix[k].map(
            (x, l) => x - unitMatrix[i][l] * scale
          );
        }
      }
    }
  ...
  for (let i = numRows - 1; i >= 0; i--) {
    if (
      varMatrix[i].every((x) => x === 0) &&
      !unitMatrix[i].every((x) => x === 0)
    ) {
...
/*
 * (deprecated) Construct two matrices (one for variables and one for units)
 * that define a system of linear equations where each equation is isomorphic to
 * a type constraint.
 *
 ...
 */
function typeConstraintsToMatrix(
  typeConstraints: [TypeConstraint, ASTNode][],
  scopes: ScopeInfo
...
/*
 * (deprecated) Given a diagonal matrix of variables and a matrix of units,
 * produce a mapping from variables to unit types.
 */
