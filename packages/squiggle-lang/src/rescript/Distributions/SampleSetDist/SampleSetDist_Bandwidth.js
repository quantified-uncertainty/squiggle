...
const iqr = (x) =>
  jstat.percentile(x, iqr_percentile, true) -
  jstat.percentile(x, iqr_percentile_complement, true);

// Silverman, B. W. (1986) Density Estimation. London: Chapman and Hall.
const nrd0 = (x) => {
  const hi = Math.sqrt(jstat.variance(x));
...
const nrd = (x) => {
  ...
  return (
    nrd_coef *
    Math.min(Math.sqrt(jstat.variance(x)), h) *
